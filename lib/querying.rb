...
def select_series_title_with_most_human_characters
  "SELECT series.title FROM series JOIN books ON books.series_id = series.id
   ...
   JOIN characters ON character_books.character_books = characters.id
   WHERE characters.species = 'human' GROUP BY series.title ORDER BY COUNT(*) DESC LIMIT 1"#WHERE characters.species = 'human' LIMIT 1
end
